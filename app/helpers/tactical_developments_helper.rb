module TacticalDevelopmentsHelper
  SKILL_LABELS = {
    "game_vision" => "Visión de juego",
    "positioning_by_role" => "Ubicación de acuerdo al rol",
    "mental_anticipation" => "Anticipación mental",
    "environment_adaptation" => "Capacidad de adaptación al entorno",
    "counteracts_opponent_actions" => "Contrarresta acciones del adversario",
    "applies_offensive_tactical_fundamentals" => "Fundamentos tácticos ofensivos",
    "applies_defensive_tactical_fundamentals" => "Fundamentos tácticos defensivos",
    "match_interval_management" => "Manejo de intervalos del partido",
    "initiative_decision_making" => "Toma de iniciativa",
    "improvises_under_unexpected_situation" => "Improvisa ante imprevistos",
    "communication_during_match" => "Habla durante el partido"
  }.freeze
  ...
  def tactical_skill_label(skill)
    SKILL_LABELS[skill.to_s] || skill.to_s.humanize
  end

  def tactical_rating_label(value)
    RATING_LABELS[value.to_s] || value.to_s.humanize
  end

  def tactical_rating_options
    [["Bien", "good"], ["Regular", "regular"], ["Insuficiente", "insufficient"]]
  end

  def tactical_trend_badge(current_score, previous_score)
  ...
  def tactical_progress_chart(records)
    ...
    grid_lines = [0, 1, 2].map do |score|
      ...
      <<~SVG
        ...
        <text x="10" y="#{y + 5}" fill="#e5e7eb" font-size="12">#{tactical_score_label(score)}</text>
      SVG
    end.join
  ...
  def tactical_score_label(score)
    case score
    when 2 then "Bien"
    when 1 then "Regular"
    else "Insuficiente"
